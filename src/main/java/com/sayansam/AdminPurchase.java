...
public class AdminPurchase {
	...
	JFrame stock = new JFrame("Admin Purchases");
	JLabel heading, sidePic, stockList;
	JScrollPane scrollableBill;
	JTable billTable;
	String billColumn[] = {"Transaction Id","Username","Total Cost","Date"};
	String billContent[][];
	...
	AdminPurchase(String username) throws Exception
	{
		AdminPurchase.username = username;
		...
		sidePic = new JLabel(new ImageIcon("pharmacygui/resources/images/Purchase.png"));
		stockList = new JLabel("Purchase of the whole store", SwingConstants.CENTER);
		...
		billTable.getColumnModel().getColumn(3).setCellRenderer( centerRenderer );
			...
			res = stmt.executeQuery("Select * from pharmacy.transaction where type='Buy';");
			
			while(res.next())
			{
				model.addRow(new Object[]{res.getInt("Id"),res.getString("Username"),res.getInt("TotalCost"),res.getString("Date")});
			}
		...
		home.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				stock.dispose();
				new LoggedInPage(AdminPurchase.username);
			}});
