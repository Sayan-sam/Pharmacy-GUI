...
public class AdminSales {
	...
	JTable billTable;
	String billColumn[] = {"Transaction Id","Username","Customer Number","Total Cost","Date"};
	String billContent[][];
	...
	AdminSales(String username) throws Exception
	{
		AdminSales.username = username;
		...
		billTable.getColumnModel().getColumn(4).setCellRenderer( centerRenderer );
			...
			res = stmt.executeQuery("Select * from pharmacy.transaction where type='Sell';");
			
			while(res.next())
			{
				model.addRow(new Object[]{res.getInt("Id"),res.getString("Username"),res.getString("compcus"),res.getInt("TotalCost"),res.getString("Date")});
			}
		...
		home.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				stock.dispose();
				new LoggedInPage(AdminSales.username);
			}});
